import { unitsHaveSameType, unitsHaveSamePolarity, isReact, polymerAfterReactions, getFullyReactedPolymerSize } from '../solution-pt1';

import inputLoader from '~root/inputLoader';

describe('day 5, part 1', () => {
  test('it must check if units have same type', () => {
    const testData: [string, string][] = [['r', 'R'], ['a', 'a'], ['b', 'X'], ['b', 'x']];
  ...
  test('it must check if units have same polarity', () => {
    const testData: [string, string][] = [['a', 'A'], ['x', 'x'], ['R', 'r'], ['R', 'R']];
  ...
  test('it must check if units reacts', () => {
    const testData: [string, string][] = [['a', 'a'], ['A', 'a'], ['x', 'b'], ['r', 'Y'], ['u', 'U']];
    const expected = [false, true, false, false, true];
  ...
  test('it must show valid polymer after all reactions', () => {
    const testData = ['aA', 'abBA', 'abAB', 'aabAAB', 'dabAcCaCBAcCcaDA'];
    const expected = ['', '', 'abAB', 'aabAAB', 'dabCBAcaDA'];
